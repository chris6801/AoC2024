...
def can_move(pos, dxdy, dmap, d_idx, prev_obs):
    ...
    if in_map:
        if dmap[target[0]][target[1]] != '#':
            return True
        elif dmap[target[0]][target[1]] == '#':
            #print('obstacle!')
            prev_obs.append([pos[0], pos[1], dy, dx])
            return False
    elif not in_map:
        return True
    return False
...
def test_path(dmap, guard_pos, dxdy, d_idx, prev_obs):
    ...
    while True:
        dy = dxdy[d_idx][0]
        dx = dxdy[d_idx][1]
        if [guard_pos[0], guard_pos[1], dy, dx] in prev_obs:
            #print("Loop detected")
            return dmap, 1
...
for obs in possible_obs:
    ...
    d_idx = 0
    prev_obs = []
    _, val = test_path(test, guard_pos, dxdy, d_idx, prev_obs)
    if val == 1:
        total += 1
